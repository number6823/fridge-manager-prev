        ...
        if (StringUtils.hasText(bearerToken) && bearerToken.startsWith("Bearer")) {
            ...
            if (jwtUtil.validateToken(token)) {
                ...
                if (user != null && user.getDeletedAt() == null) {
                    ...
                    // Spring Security 는 지금 들어온 사용자 권한 정보를 SimpleGrantAuthority 객체에 저장함
                    ...
                    // UsernamePasswordAuthenticationToken 이라는 타입은 3칸(매개변수)를 기록할 수있고
                    // 첫번째 칸 (매개변수) 에는 사용자 ID, 두번쨰 칸(매개변수)에는 그 외 정보 객체, 세번쨰 칸(매개변수)에는 ROLE 담김
